...
export QUERY="SELECT count() from merge_tree.even_time_order_func_batch group by toYYYYMM(time)"
python selecter.py

export QUERY="SELECT count() from merge_tree.even_time_order_func_single group by toYYYYMM(time)"
python selecter.py
